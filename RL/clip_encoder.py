...
import ipdb
import math
#from .caption_clipcap import ClipCaptionModel, generate_beam, generate_nucleus, generate_diverse_beam, generate_top_k, generate_greedy
import clip
...
class CLIPVisionTower(nn.Module):
    ...
    def token_prune_merge_advanced_plus(self, images, prompt, if_adaptive=True, reduction_ratio = 1/8):
        ...
        if self.training:
            ipdb.set_trace()
            max_num_text = 25    # 60 for 104 tokens
        ...
        for i in range(B):
            ...
            supplement_token.append(proj_feature[i][remain_idx].unsqueeze(dim=0))    # (1, 540, C)
            
            
        supplement_token = torch.cat(supplement_token, dim=0)   # (B, 540, 768)
        #ipdb.set_trace()
    ...
    def forward(self, images, prompt):
        if type(images) is list:
            ...
            cls_features, image_features, x_others, supplement_token = self.token_prune_merge_advanced_plus(images, prompt, if_adaptive=True, reduction_ratio=1/8) 
