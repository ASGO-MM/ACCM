...
# from .flops_utils import RelTRFLOPsCounter
...
class RelTR(nn.Module):
    ...
    @profile
    # def forward(self, samples: NestedTensor, feat_for_sg: torch.Tensor):
    def forward(self, samples: NestedTensor, feat_for_sg: torch.Tensor, question_embeds: torch.Tensor):
        ...
        """
        # samples: torch.Size([1, 3, 800, 800])
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)    # torch.Size([1, 3, 800, 800]), torch.Size([1, 800, 800])
        features, pos = self.backbone(samples, feat_for_sg)   #  features: [torch.Size([1, 2048, 50, 50]), torch.Size([1, 50, 50])]  
        src, mask = features[-1].decompose()
        assert mask is not None
        q_input = question_embeds.unsqueeze(-1).unsqueeze(-1)
        q_input = q_input.to(dtype=self.input_proj.weight.dtype)
        q_embed = self.input_proj(q_input)
        q_embed = q_embed.flatten(1)
        q_embed = q_embed.unsqueeze(0)
        hs, hs_t, so_masks, _ = self.transformer(
            self.input_proj(src), 
            mask, 
            self.entity_embed.weight,
            self.triplet_embed.weight, 
            pos[-1], 
            self.so_embed.weight,
            question_embeds=q_embed  
        )
