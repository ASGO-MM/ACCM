...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed, question_embeds=None):
        ...
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed, question_embeds=question_embeds)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        if question_embeds is not None:
            ...
            src_concat = torch.cat([src, question_embeds], dim=0)
            q_k_img = self.with_pos_embed(src, pos)          
            q_k_txt = question_embeds
            q_k_concat = torch.cat([q_k_img, q_k_txt], dim=0)

            if src_key_padding_mask is not None:
                mask_pad = torch.zeros((src.shape[1], question_embeds.shape[0]), 
                                       dtype=torch.bool, device=src.device)
                mask_concat = torch.cat([src_key_padding_mask, mask_pad], dim=1)
            else:
                mask_concat = None
            src2 = self.self_attn(q_k_concat, q_k_concat, value=src_concat, attn_mask=src_mask,
                                  key_padding_mask=mask_concat)[0]
    ...
    def forward(self, src,
        ...
        return self.forward_post(src, src_mask, src_key_padding_mask, pos, question_embeds=question_embeds)
