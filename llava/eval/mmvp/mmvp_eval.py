...
def process(line, args, tokenizer, image_processor, model_config, images, img_paths):
    ...
    pil_image = transform(input_image).unsqueeze(0)

    return input_ids, image_tensor, image_size, prompt, text_prompt, img_path, pil_image
...
def eval_model(args):
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, image_tensor, image_sizes, prompt, text_prompt, img_path, pil_image = process(line, args, tokenizer, image_processor, model.config, images, img_paths)
        gt_answer = line["answer"]
        input_ids = input_ids.to(device='cuda', non_blocking=True)

        img_path_tuple = (img_path,)
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                ...
                images_224=None,
                img_path=img_path_tuple,
                pil_image=pil_image.to(device='cuda', non_blocking=True),
