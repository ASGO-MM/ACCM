...
def process(line, args, tokenizer, image_processor, model_config, images):
    ...
    return input_ids, image_tensor, image_size, prompt, text_prompt
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, _, context_len = load_pretrained_model(model_path, args.model_base, model_name)
    model.post_config(args)
    #
    model.to(dtype=torch.bfloat16)
    #model.to("cuda")       # for eval selector !!!
    #
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, image_tensor, image_sizes, prompt, text_prompt = process(line, args, tokenizer, image_processor, model.config, images)
        #print(text_prompt)
        gt_answer = line["answer"]
        input_ids = input_ids.to(device='cuda', non_blocking=True)
        #ipdb.set_trace()
        #image_tensor = [image_tensor[0].to(dtype=torch.bfloat16).cuda()]
